...
if check.lower() == "y":
    ...
    with smtplib.SMTP_SSL('smtp.gmail.com',465,context = context) as smtp:
        ...
        directory = [f for f in os.listdir() if '.' not in f and f != "__pycache__"]
        for d in directory:
            for person in os.listdir(d):
                ...
                attach.add_header('Content-Disposition','attachment',filename=f"{mouth[time.strftime('%B')]} {int(time.strftime('%Y'))+543}")
                msg.attach(attach)
                os.remove(f"{person}.png")
                smtp.sendmail(sender,person.split(',')[1][0:-4],msg.as_string())
                print(f'Mail has senfd to {person.split(",")[0]}')